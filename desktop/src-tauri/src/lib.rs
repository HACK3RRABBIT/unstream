...
fn spawn_backend(
    ...
    cmd.arg("--host")
        ...
        // The two TTL/budget knobs above are not the only clocks the sweeper
        // runs on: finished files also carry their own delivered/undelivered
        // retention windows (1h / 15min by default — right for a server that
        // must not accumulate strangers' media, wrong for someone's own
        // machine). Left unset, a desktop download — anime episode or music
        // track alike — would vanish within the hour even with the above set
        // to "keep forever".
        .env("DOWNLOAD_RETENTION_HOURS", "0")
        .env("DELIVERED_RETENTION_HOURS", "0")
        ...
        // Anime episodes are 100-700MB, so this pool stays smaller than the
        // audio one even here — it bounds how many episodes download at
        // once, not how fast one does (that's ANIME_HLS_CONCURRENCY below).
        // A home connection has more headroom than a shared server, but the
        // providers on the other end (Nyaa, the HLS sidecar's sources) are
        // still the same ones every self-hoster shares — move this in small
        // steps, same reasoning as DOWNLOAD_WORKERS in backend/app/jobs.py.
        .env("ANIME_DOWNLOAD_WORKERS", "3")
        // HLS fragments within one episode, fetched in parallel — serial
        // fragments (not bandwidth) dominate an episode's download time, so
        // this is the lever that actually uses a fast home connection's
        // throughput instead of a datacenter's fair-share limits.
        .env("ANIME_HLS_CONCURRENCY", "12")
        .env(
            "LYRICS_DB_PATH",
            app_data_dir
                .join("lyrics.db")
