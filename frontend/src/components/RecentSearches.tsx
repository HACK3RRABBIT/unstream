...
/** The last few things the user asked for, as chips under the hero.
 *
 *  The homepage otherwise offers nothing to click: an empty input and a
 *  paragraph. These give a returning visitor a one-tap way back to whatever
 *  they were doing — which matters most on a phone, where retyping an album
 *  name is the expensive part. */
        ...
        {items.map((item) => (
          <li key={item.input}>
            <button
              ...
              dir="auto"
              // A pasted URL is unreadable at chip width, so a link shows its
              // host and its icon instead of a truncated https://…
              title={item.input}
...
/** "open.spotify.com/album/…" reads better on a chip than the whole URL. */
