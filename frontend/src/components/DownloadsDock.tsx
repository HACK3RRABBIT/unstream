...
function TrackLine({ entry, state }: { entry: DownloadEntry; state: JobTrack }) {
  ...
  return (
    <li className="flex items-center gap-2.5 px-4 py-1.5">
      {state.status === 'done' ? (
        <Check className="size-3.5 shrink-0 text-lime-flash" />
      ) : state.status === 'error' ? (
  ...
  return (
    <div className="border-b border-ink-800 last:border-b-0">
      <div className="flex items-center gap-3 px-4 pt-3 pb-2">
        {entry.cover_url ? (
          <img src={entry.cover_url} alt="" className="size-9 shrink-0 rounded-md object-cover" />
        ) : (
          <div className="size-9 shrink-0 rounded-md bg-ink-800" />
        )}
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13.5px] font-medium text-ink-100">{entry.name}</p>
        ...
        {finished && (
          <button
            ...
            className="grid size-7 shrink-0 place-items-center rounded-lg text-ink-400 transition hover:bg-ink-800 hover:text-ink-100"
          >
      ...
      <div className="mx-4 h-0.5 overflow-hidden rounded-full bg-ink-800">
        <div
          className="h-full rounded-full bg-lime-flash transition-[width] duration-500"
      ...
      <ul className="max-h-44 overflow-y-auto py-1.5">
        ...
        {!job && (
          <li className="flex items-center gap-2.5 px-4 py-1.5 text-[13px] text-ink-400">
    ...
    <div className="fixed right-5 bottom-5 z-50 flex flex-col items-end gap-3">
      {panelOpen && (
        <section
          ...
          className="flex w-[min(24rem,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-2xl border border-ink-700 bg-ink-900 shadow-2xl shadow-black/60"
        >
          <header className="flex items-center justify-between border-b border-ink-800 px-4 py-3">
            <h2 className="text-[11px] font-semibold tracking-[0.14em] text-ink-400 uppercase">
              Downloads
            </h2>
            <span className="text-[12px] text-ink-400 tabular-nums">
              {activeCount > 0
      ...
      <button
        ...
        className="relative grid size-14 place-items-center rounded-full bg-lime-flash text-lime-ink shadow-lg shadow-black/40 transition hover:bg-lime-soft focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-lime-flash"
      >
        ...
        {activeCount > 0 && !panelOpen && (
          <span className="absolute -top-0.5 -right-0.5 grid min-w-5 place-items-center rounded-full border border-lime-flash bg-ink-950 px-1 text-[11px] font-semibold text-lime-flash tabular-nums">
            {activeCount}
          </span>
        )}
      </button>
