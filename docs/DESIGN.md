...
- **Preload `href`s must match the CSS `url()`s exactly**, or browsers download every font twice. `sw.template.js` precaches the same five files, and `cache.addAll` rejects atomically — one stale filename there and the service worker never installs at all.
